...
CenterDlg::CenterDlg(mainthread *pmainthr, config *pcfg, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::CenterDlg)
{
    ui->setupUi(this);
    ...
    ui->chkFTPImgGray->setVisible(false);
    ui->leFTPPath->setVisible(false);
    ui->leLaneID->setMaxLength(3);
    ui->leManagerID->setMaxLength(4);
    ui->leDirection->setMaxLength(1);
...
void CenterDlg::configload()
{
    ...
    ui->leFTPSavePath->setText(commonvalues::FTPSavePath);
}
...
void CenterDlg::configsave()
{
    ...
    if( centercount > 0)
    {
        for(int i=0; i < centercount; i++)
        {
            ...
            cfg->setuint(title,"ProtocolType",commonvalues::center_list[i].protocol_type);
    ...
    cfg->set("CENTER","FTPSavePath",commonvalues::FTPSavePath);
...
void CenterDlg::on_btnInsert_clicked()
{
    ...
    int iprotocoltype = ui->cbCommType->currentIndex();
    ...
    cinfo.protocol_type = iprotocoltype;
    ...
    commonvalues::FTPSavePath = ui->leFTPSavePath->text();
...
void CenterDlg::on_btnModify_clicked()
{
    ...
    if( irow >= 0)
    {
        ...
        commonvalues::center_list[irow].protocol_type = ui->cbCommType->currentIndex();
